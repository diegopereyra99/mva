...
long int id_node(int x, int y, int nx, int ny, int d){
    return y + ny*(x + nx*d); 
}
    ...
    for (int x=0; x<nx; x++){
        for (int y=0; y<ny; y++){
            ...
            for (int d=0; d<nd; d++){
                ...
                long int node_id = id_node(x, y, nx, ny, d);
                
                float w_pj = INF;
                if (xx + disparity > win && xx + disparity < I2.width() - win){ // Check no overflow 
                    float zncc_score = zncc(I1, I1M, I2, I2M, xx, yy, xx + disparity, yy);
                    float Dpj = wcc*rho(zncc_score);
                    w_pj = Dpj + Kp; 
                }

                if (d == 0){ // source
                    G.add_tweights(node_id, w_pj, 0);
                } else { 
                    long int prev_node_id = id_node(x, y, nx, ny, d-1);
                    G.add_edge(prev_node_id, node_id, w_pj, 0);
                    
                    if (d == nd-1){ // sink
                        G.add_tweights(node_id, 0, w_pj);
                    }
                }

                // Weights for edges with neighbor pixels
                if(x < nx - 1){ 
                    long int right_id = id_node(x+1, y, nx, ny, d);
                    G.add_edge(node_id, right_id, lambda, lambda);
                }
                if(y < ny -1){ 
                    long int down_id = id_node(x, y+1, nx, ny, d);
                    G.add_edge(node_id, down_id, lambda, lambda);
                }
            }
        }
    }
